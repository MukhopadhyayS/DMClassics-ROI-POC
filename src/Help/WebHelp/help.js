	...
	helpSystem.Load(function ()
	{
	    helpSystem.LookupCSHID(id, function (idInfo)
	    {
	        function OnGetSkinComplete()
	        {
	            ...
	            //CAF should not try and open new windows
				if (mpf.getApplication().integrationContext && mpf.getApplication().integrationContext.cafenabled) {
					var title = mpf.getApplication().i18nManager.getString('help.window.title', 'McKesson Help', 'container');
					mpf.getApplication().openLinkInExtWindow('helpViewerWindowId', 'contentArea', cshString, 'text/javascript', title, false, 600, 800);
				} else {
