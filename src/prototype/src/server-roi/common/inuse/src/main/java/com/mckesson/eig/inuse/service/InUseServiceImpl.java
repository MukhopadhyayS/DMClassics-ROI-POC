...
public class InUseServiceImpl implements InUseService {
    ...
    private static final String QUARTZ_GROUP_NAME = "com.mckesson.eig.inuse";
    private static final String QUARTZ_JOB_NAME = "com.mckesson.eig.inuse.service.InUseQuartzJob";
    private static final String AUTHENTICATED_USER = "authenticated_inuse_user";
    ...
    public void initService() throws SchedulerException {
        ...
        synchronized (_sem) {
            ...
            try {
                ...
                ((JobDetailImpl) detail).setGroup(QUARTZ_GROUP_NAME);
                ((JobDetailImpl) detail).setName(QUARTZ_JOB_NAME);
                ((JobDetailImpl) detail).setJobClass(InUseQuartzJob.class);
                ((JobDetailImpl) detail).setDurability(true);
    ...
    */
    ...
    private void cancelTimerForRecord(InUseRecord record) throws SchedulerException {
        ...
        SimpleTrigger trigger = new SimpleTriggerImpl(name, QUARTZ_GROUP_NAME);
        ((SimpleTriggerImpl)trigger).setJobGroup(QUARTZ_GROUP_NAME);
        ((SimpleTriggerImpl)trigger).setJobName(QUARTZ_JOB_NAME);
        ((SimpleTriggerImpl)trigger).setRepeatCount(0);
    ...
    private void createTimerForRecord(InUseRecord record) throws SchedulerException {
        ...
        SimpleTrigger trigger = new SimpleTriggerImpl(name, QUARTZ_GROUP_NAME, new Date(fireTime));
        ((SimpleTriggerImpl)trigger).setJobGroup(QUARTZ_GROUP_NAME);
        ((SimpleTriggerImpl)trigger).setJobName(QUARTZ_JOB_NAME);
        ((SimpleTriggerImpl)trigger).setRepeatCount(0);
    ...
    private void updateTimerForRecord(InUseRecord record) throws SchedulerException {
        ...
        ((SimpleTriggerImpl)trigger).setJobName(QUARTZ_JOB_NAME);
        ((SimpleTriggerImpl)trigger).setRepeatCount(0);
        _scheduler.rescheduleJob(trigger.getKey(), trigger);
    }
