...
* 	have been supplied.
*****************************************************************
* SQL script Name: Requests_Converted_As_Unbillable_But_Should_Not.sql
* Description: The script lists the ROI_RequestMain_seq of classic requests that got converted into 'Un-billable' requests,
*			   but should not. 
*
* NOTES: 
* Revision History:
...
SELECT ROI_RequestMain_Seq from xmlRequests
JOIN  cabinet.dbo.ROI_RequestCore core ON xmlRequests.ROI_RequestMain_Seq = core.ROI_RequestCore_Seq 
WHERE core.Unbillable = 1 AND xmlRequests.ROI_RequestMain_Seq NOT IN (

SELECT unbillables.ROI_RequestMain_Seq as 'Requests IDs that we converted as un-billable and they should not' FROM (
