...
function CMCTokenizer()
{
    ...
    function ReadNextToken()
    {
        ...
        else if ( CMCIsWhiteSpace( c ) )
        {
            for ( c = Peek(); CMCIsWhiteSpace( c ); c = Peek() )
            {
                Read();
                tokenText += c;
            }
        ...
        else if (CMCIsTermSeparator(c))
        {
            Read();
            token = new CMCToken(c, CMCToken.TermSeparator);
        }
        else
        {
            for ( c = Peek(); CMCIsNameChar( c ); c = Peek() )
            {
                Read();
                tokenText += c;
            }
...
function CMCParser( str )
{
                        ...
                        for (var j = 0; j < gSearchDBs.length; j++)
                        {
                            ...
                            if (searchDB.SearchType == "NGram")
                            {
                                for (var k = 0; k < term.length - searchDB.NGramSize + 1; k++)
                                {
                                    ...
                                    var stem = searchDB.Stemmer.stemWord(subText);
                            ...
                            {
                                var stem = searchDB.Stemmer.stemWord(term);
    ...
    {
        for ( ; Peek() == CMCToken.WhiteSpace || Peek() == CMCToken.TermSeparator; )
        {
            EatToken();
        }
    }
...
function CMCNode( token, op1, op2 )
{
    ...
    this.EvaluateWord = function (buildWordMap, buildPhraseMap, OnCompleteFunc)
    {
        ...
        var stemmer = gSearchDBs[0].Stemmer;
        var stems = new CMCDictionary();
        var startStem = stemmer.stemWord(tokenText);
        ...
        function OnLoadChunkComplete()
        {
            ...
            if (completed == length)
            {
                for (var i = 0; i < terms.length; i++)
                {
                    ...
                    var stems = new CMCDictionary();
                    var startStem = stemmer.stemWord(term);
...
function CMCIsNameChar(c) {
    ...
    else if (CMCIsWhiteSpace(c)) { return false; }
    else if (CMCIsTermSeparator(c)) { return false; }
    else { return true; }
}

function CMCIsWhiteSpace(c) {
    if (!c) {
        return false;
    }
    else if (c == " ") {
...
function CMCIsTermSeparator(c) {
    return (String.IsPunctuation(c) && c != '\'' && c != '_')
        || c == '>' || c == '<' || c == '=';
}
...
function SplitPhrase( text )
{
	...
	{
	    terms = String.Split(text, function (c) {
	        return CMCIsWhiteSpace(c) || CMCIsTermSeparator(c);
	    });
	}
