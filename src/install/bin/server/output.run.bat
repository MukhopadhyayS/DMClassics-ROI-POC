...
rem clear the jboss_home environment vaiable
set JBOSS_HOME=
...
REM modified for Print issue, while run as adminstrator
CD %DIRNAME%
...
if "x%JAVA_OPTS%" == "x" (
  set "JAVA_OPTS=-Dprogram.name=%PROGNAME% -Djboss.bind.address=0.0.0.0"
) else (
  set "JAVA_OPTS=-Dprogram.name=%PROGNAME% -Djboss.bind.address=0.0.0.0 %JAVA_OPTS%"
)
...
rem ------------------------------------------
rem CR # 349,019
rem Just to start the JVM with minimum heap size and gradually increased it.
...
rem **************************************************
rem Added for HPFW portal
SET JAVA_OPTS=%JAVA_OPTS% -Djavax.security.auth.useSubjectCredsOnly=false
SET JAVA_OPTS=%JAVA_OPTS% -Djava.security.krb5.conf=%JBOSS_HOME%\server\default\deploy\portal.war\WEB-INF\classes\config\authentication\krb5.conf
rem **************************************************

rem **************************************************
rem CR 346694
rem Delete cached tmp and work subfolders ***
echo Deleting temporary folder and subfolders
rd "%JBOSS_HOME%\server\default\tmp" /s /q 
rd "%JBOSS_HOME%\server\default\work" /s /q 
rd "%JBOSS_HOME%\server\default\data" /s /q 
echo Temporary folders and subfolders deleted
rem **************************************************
