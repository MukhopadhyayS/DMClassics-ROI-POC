...
public class TestGetOwnedProcessesInfo extends
com.mckesson.eig.workflow.test.AbstractWorkflowTestCase {
    ...
    public void testGetOwnedProcessesInfoWithExpiredProcesses() {

        ProcessInfoList processInfoList = _processListService.getOwnedProcessesInfo(1,
                true, _owners, _variableList);
        assertTrue(processInfoList.getProcessList().size() > 0);
    }

    public void testGetOwnedProcessesInfoWithOutExpiredProcesses() {
        ProcessInfoList processInfoList = _processListService.getOwnedProcessesInfo(1,
                false, _owners, _variableList);
        assertTrue(processInfoList.getProcessList().size() == 0);
    }

    public void testGetOwnedProcessesInfoWithInvalidApplicationID() {

        try {
            _processListService.getOwnedProcessesInfo(-1, true, _owners, _variableList);
    ...
    public void testGetOwnedProcessesInfoInfoWithInvalidProcessType() {
    ...
    public void testGetOwnedProcessesInfoWithNullProcessType() {

        try {
            _processListService.getOwnedProcessesInfo(1, true, _owners, null);
        ...
        try {
            _processListService.getOwnedProcessesInfo(1, true, _owners, variableList);
            fail("Should have thrown EC_PROCESS_TYPE_NOT_SPECIFIED exception");
        } catch (WorkflowException we) {
            assertEquals(ProcessEC.EC_PROCESS_TYPE_NOT_SPECIFIED, we
                    .getErrorCode());
    ...
    public void testGetOwnedProcessesInfoWithNullOwners() {
        try {
            _processListService.getOwnedProcessesInfo(1, true, null, _variableList);
    ...
    public void testGetOwnedProcessesInfoWithInvalidOwners() {
        try {
            _processListService.getOwnedProcessesInfo(1, true,
