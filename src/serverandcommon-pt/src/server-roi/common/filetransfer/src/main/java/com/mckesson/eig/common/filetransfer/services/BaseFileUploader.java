...
public class BaseFileUploader extends HttpServlet implements IUploadContent {
    ...
    private static final OCLogger LOG = new OCLogger(BaseFileUploader.class);

    public static final int SUCCESS = 0;
    public static final int FAILED  = 1;
    public static final int RESEND  = 2;
    public static final int SESSION = 3;
    private static final long MAX_UPLOAD_FILE_BYTES = 500000000;
    ...
    private HtmlCustomCodec htmlCustomCodec = new HtmlCustomCodec();
    ...
    protected int writeFileBuffered(BaseFileTransferData data, InputStream fis, String strDirPath,
        ...
        try {
           ...
           while (lWritten < nDataLength) {
              ...
              fos.write(getValidFileContent(strFileName, buffer, CHUNK_SIZE, false), 0, nAmountRead);
    ...
    private String encodeMessage(String message) {
        message = htmlCustomCodec.encode(message);
        return message;
    }
 ...
 * @throws IOException 
    ...
    */
    ...
    public byte[] getValidFileContent(String context, byte[] input, int maxBytes, boolean allowNull) throws Exception {
    	if (input == null || input.length == 0) {
    	   if (allowNull) 
    		   return null;
    	}
    	long esapiMaxBytes = MAX_UPLOAD_FILE_BYTES;
    	if (input.length > esapiMaxBytes ) 
    		throw new Exception( context + ": Invalid file content can not exceed " + esapiMaxBytes + " bytes");
    	if (input.length > maxBytes ) 
    		throw new Exception( context + ": Invalid file content can not exceed " + maxBytes + " bytes" );
        return input;
    }
