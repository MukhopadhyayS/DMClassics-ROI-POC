...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[wfe_DeletePacketControlDSRPT]') AND type in (N'P'))
DROP PROCEDURE [dbo].[wfe_DeletePacketControlDSRPT]
GO

/************************************************************************************************************************
* Stored Procedure	: dbo.wfe_DeletePacketControlDSRPT
...
* Purpose		: Workflow
*
* Description		: This stored procedure checks the Coding Discharge Summary Report Packet Controls for the
...
create PROCEDURE [dbo].[wfe_DeletePacketControlDSRPT]  (
...
-- Custom Code Begins
	...
	Begin
		   ...
		   where userid = 'HIM: Coding Wait for Discharge Summary'
		   and encounter = @account_ID
		   and facility = @facility_ID

		   delete from cabinet..assignments 
		   where userid = 'HIM: Coding Wait for Discharge Summary Scanned'
		   and encounter = @account_ID
		   and facility = @facility_ID
		   ...
		   INSERT INTO [AssignmentAuditEvent]
            ...
            VALUES
                  ...
                  GETDATE(), '1', 'sp_wfe_DeletePacketControlDSRPT', (select PURPOSE from ASSIGNMENTS where ASSIGNMENT_ID=@Assignment_Id));
...
GRANT EXECUTE ON [dbo].[wfe_DeletePacketControlDSRPT] TO [IMNET] AS [dbo]
GO
