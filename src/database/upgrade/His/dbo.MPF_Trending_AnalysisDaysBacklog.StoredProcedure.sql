...
* 	have been supplied.
*****************************************************************
* Procedure Name: [MPF_Trending_AnalysisDaysBacklog]
* Description:   
*
* NOTES: 
* Revision History:
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[MPF_Trending_AnalysisDaysBacklog]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[MPF_Trending_AnalysisDaysBacklog]
GO

/*
	  UC5933 - View Trending Analysis Days Backlog Measure
	...
	==============================================================================================================================
*/
CREATE PROCEDURE [dbo].[MPF_Trending_AnalysisDaysBacklog]
...
DELETE Trending_AnalysisDaysBacklog where TrendingDate < @TrendRetentionDate
...
SET @TrendDate = (select CONVERT(smalldatetime, CONVERT(char(10), (select max([TrendingDate]) from Trending_AnalysisDaysBacklog WITH (NOLOCK)) + 1, 101)))
...
DELETE Trending_AnalysisDaysBacklog where TrendingDate >= @TrendDate
...
SET  @sqlcmd = 'del ' + @HBI_TargetDir + '\MPF_Trending_AnalysisDaysBacklog.txt'
...
-- Preload with current assignments in Analysis queues.
...
BEGIN
	...
	BEGIN
		...
		WHERE c.assignmentEventDate >= DATEADD(day,0,@AsOfDateVar) AND c.assignmentEventDate < DATEADD(day,1,@AsOfDateVar) 
		      AND c.QueueTypeName='Analysis' AND (c.WorkflowEventName='Complete' ) 
...
SET  @sqlcmd = ' bcp [his].[dbo].Trending_AnalysisDaysBacklog out ' + rtrim(@HBI_TargetDir) + '\MPF_Trending_AnalysisDaysBacklog.txt -c -T -S' + @@SERVERNAME
...
IF @Success = 1
	RAISERROR ('bcp of Trending_AnalysisDaysBacklog was not successful', 10, 1)
...
GRANT exec on [dbo].[MPF_Trending_AnalysisDaysBacklog] to [IMNET]
go
