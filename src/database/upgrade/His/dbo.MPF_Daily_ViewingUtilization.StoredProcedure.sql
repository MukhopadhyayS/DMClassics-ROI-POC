...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[MPF_Daily_ViewingUtilization]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[MPF_Daily_ViewingUtilization]
GO
...
* 	have been supplied.
*****************************************************************
* Procedure Name: [MPF_Daily_ViewingUtilization]
* Description:   
*
* NOTES: 
* Revision History:
...
*/
/*
	UC5952 - View Daily Viewing Utilization Measure

	Utilization of MPF Viewing
	...
	Occurred					AUDIT		AUDIT_TRAIL				Datetime when viewing				Occurred
																	event occurred
	...
	UserInstanceId				CABINET		DOC_RECORDVIEW_VIEW		Total number of users in a			TotalUsersWithAccess
...
CREATE PROCEDURE [dbo].[MPF_Daily_ViewingUtilization]
...
SET  @sqlcmd = 'del ' + @HBI_TargetDir + '\MPF_Daily_ViewingUtilization.txt'
EXEC @Success =  master..xp_cmdshell @sqlcmd, NO_OUTPUT

IF OBJECT_ID('tempdb..##HBI_SUBSET_DAILY_VIEWINGUTILIZATION') IS NOT NULL
DROP TABLE ##HBI_SUBSET_DAILY_VIEWINGUTILIZATION
	...
	[TotalUsersWithAccess] = 
		CASE
			WHEN ROW_NUMBER() over (partition by Facility order by occurred) = 1
				THEN (select count(distinct rv.userInstanceId)
						FROM [cabinet].[dbo].[doc_recordview_view] rv with (nolock)
						JOIN [cabinet].[dbo].[User_Facility] uf  with (nolock) ON rv.userInstanceId = uf.USER_ID
						JOIN [cabinet].[dbo].[USERS] u  with (nolock) on uf.USER_ID = u.UserInstanceId and u.AccountDisabled = 0
						WHERE uf.FACILITY = aud_trail.FACILITY)
			ELSE 0
		END
into ##HBI_SUBSET_DAILY_VIEWINGUTILIZATION
from [audit].[dbo].[AUDIT_TRAIL] aud_trail with (nolock)
left join [cabinet].[dbo].[USERS] users with (nolock)
...
SET  @sqlcmd = ' bcp ##HBI_SUBSET_DAILY_VIEWINGUTILIZATION out ' + rtrim(@HBI_TargetDir) + '\MPF_Daily_ViewingUtilization.txt -c -T -S' + @@SERVERNAME

EXEC @Success =  master..xp_cmdshell @sqlcmd --, NO_OUTPUT

IF @Success = 1
	RAISERROR ('bcp of ##HBI_SUBSET_DAILY_VIEWINGUTILIZATION was not successful', 10, 1)
...
GRANT exec on [dbo].[MPF_Daily_ViewingUtilization] to [IMNET]
go