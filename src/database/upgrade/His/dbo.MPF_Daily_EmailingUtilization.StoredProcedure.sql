...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[MPF_Daily_EmailingUtilization]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[MPF_Daily_EmailingUtilization]
GO
...
* 	have been supplied.
*****************************************************************
* Procedure Name: [MPF_Daily_EmailingUtilization]
* Description:   
*
* NOTES: 
* Revision History:
...
* 	Richard Brightwell		12/20/2012	Updated to show GLOBAL for global documents
...
*/
/*
	UC5952 - View Daily Emailing Utilization Measure

	Utilization of MPF Emailing
	...
	Occurred					AUDIT		AUDIT_TRAIL				Datetime when Emailing				Occurred
																	event occurred
...
CREATE PROCEDURE [dbo].[MPF_Daily_EmailingUtilization]
...
SET  @sqlcmd = 'del ' + @HBI_TargetDir + '\MPF_Daily_EmailingUtilization.txt'
EXEC @Success =  master..xp_cmdshell @sqlcmd, NO_OUTPUT

IF OBJECT_ID('tempdb..##HBI_SUBSET_DAILY_EMAILINGUTILIZATION') IS NOT NULL
DROP TABLE ##HBI_SUBSET_DAILY_EMAILINGUTILIZATION
...
SELECT
	...
	[TotalUsersWithAccess] = 
		CASE
			...
			ELSE 0
		END
into ##HBI_SUBSET_DAILY_EMAILINGUTILIZATION
...
SET  @sqlcmd = ' bcp ##HBI_SUBSET_DAILY_EMAILINGUTILIZATION out ' + rtrim(@HBI_TargetDir) + '\MPF_Daily_EmailingUtilization.txt -c -T -S' + @@SERVERNAME

EXEC @Success =  master..xp_cmdshell @sqlcmd --, NO_OUTPUT

IF @Success = 1
	RAISERROR ('bcp of ##HBI_SUBSET_DAILY_EMAILINGUTILIZATION was not successful', 10, 1)
...
GRANT exec on [dbo].[MPF_Daily_EmailingUtilization] to [IMNET]
go