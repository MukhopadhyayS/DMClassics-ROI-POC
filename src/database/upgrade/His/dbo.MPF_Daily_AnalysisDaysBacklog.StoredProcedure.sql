...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[MPF_Daily_AnalysisDaysBacklog]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[MPF_Daily_AnalysisDaysBacklog]
GO
...
* 	have been supplied.
*****************************************************************
* Procedure Name: [MPF_Daily_AnalysisDaysBacklog]
* Description:   
*
* NOTES: 
* Revision History:
...
*   Iryna Politykina		04/08/2013  Fix per Cr 379167 : Daily Analysis Backlog - LOS variables not correct
...
*/
/*
	UC5926 - View Daily Analysis Days Backlog Measure

	backlog of encounters waiting for Analysis
	...
	CALCULATED FIELD			HIS			ENCOUNTESR				CONVERT(VARCHAR(10),e.Discharged,101) DischargedDateShortText
	==============================================================================================================================
*/

CREATE PROCEDURE [dbo].[MPF_Daily_AnalysisDaysBacklog]
...
SET  @sqlcmd = 'del ' + @HBI_TargetDir + '\MPF_Daily_AnalysisDaysBacklog.txt'
EXEC @Success =  master..xp_cmdshell @sqlcmd, NO_OUTPUT

IF OBJECT_ID('tempdb..##HBI_SUBSET_DAILY_ANALYSISDAYSBACKLOG') IS NOT NULL
DROP TABLE ##HBI_SUBSET_DAILY_ANALYSISDAYSBACKLOG
...
SELECT
	...
	(SELECT TOP 1 UNIT FROM dbo.LOCATOR WITH (NOLOCK) WHERE FOLDER=e.ENCOUNTER and FACILITY=e.FACILITY ORDER BY occurred DESC )
	 AND SET_ID=(SELECT TOP 1 UNIT_SET FROM cabinet.dbo.FACILITY_FILE WITH (NOLOCK) WHERE FACILITY_CODE=e.FACILITY)), 'Undefined') AS MedicalUnitName, 
...
INTO ##HBI_SUBSET_DAILY_ANALYSISDAYSBACKLOG	
...
JOIN (SELECT DISTINCT UserID, ENCOUNTER, FACILITY from cabinet.dbo.Assignments assignments WITH (NOLOCK)
			...
			JOIN cabinet.dbo.QueueTypeLov qtlov WITH (NOLOCK) ON qtlov.QueueTypeLovID = def.QueueTypeLovID WHERE QueueTypeName='Analysis' AND assignments.ASSIGNED < DATEADD(day,datediff(day,0,getdate()),0))
...
WHERE
	...
	AND c.QueueTypeName='Analysis' AND (c.WorkflowEventName='Complete' OR c.WorkflowEventName='Delete' )
	...
	AND c.assignment_id NOT IN (SELECT assignment_id FROM cabinet.dbo.AssignmentAuditEvent WITH (NOLOCK) 
								...
								AND QueueTypeName='Analysis'  AND (WorkflowEventName='Create' OR WorkflowEventName='Reassign To' OR WorkflowEventName='ReassignTo')) -- excludes assignments that were created and completed yesterday

SET  @sqlcmd = ' bcp ##HBI_SUBSET_DAILY_ANALYSISDAYSBACKLOG out ' + rtrim(@HBI_TargetDir) + '\MPF_Daily_AnalysisDaysBacklog.txt -c -T -S' + @@SERVERNAME
...
IF @Success = 1
	RAISERROR ('bcp of ##HBI_SUBSET_DAILY_ANALYSISDAYSBACKLOG was not successful', 10, 1)
...
GRANT exec on [dbo].[MPF_Daily_AnalysisDaysBacklog] to [IMNET]
go