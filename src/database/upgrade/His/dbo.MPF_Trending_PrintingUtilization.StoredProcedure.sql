...
/*
 	Procedure Name: [MPF_Trending_PrintingUtilization]
 	Description:   
	...
	Srinivasan Ramaswamy		01/02/2013		Replaced TotalUsers who printed to user who printed
	...
	Iryna Politykina			05/01/2013		Inserted wint(NoLock) in select statement(s)
	Srinivasan Ramaswamy		05/09/2013		While counting users with Print Access from 'User_Security' table, query now ensures that that user is present in 'User_Facility' table as well
...
/*
	UC5955 - View Trending Printing Utilization User Measure
	...
	Occurred			AUDIT		AUDIT_TRAIL				Datetime when Printing						date
	UserInstanceId		AUDIT		AUDIT_TRAIL				User who printed				 			UserInstanceId
...
*/
	...
	IF EXISTS ( 
		...
		FROM 
			sys.objects 
		WHERE 
			object_id = OBJECT_ID(N'[dbo].[MPF_Trending_PrintingUtilization]') AND 
			type in (N'P')
	)
	DROP PROCEDURE [dbo].[MPF_Trending_PrintingUtilization]
	GO
...
*/
	...
	CREATE PROCEDURE 
		[dbo].[MPF_Trending_PrintingUtilization]
	...
	BEGIN TRY
		...
		SET  @sqlcmd = 'del ' + @HBI_TargetDir + '\MPF_Trending_PrintingUtilization.txt'
		...
		SET  @sqlcmd = ' bcp [his].[dbo].[Trending_PrintingUtilization] out ' + 
			rtrim(@HBI_TargetDir) + 
			'\MPF_Trending_PrintingUtilization.txt -c -T -S' + @@SERVERNAME
		...
		IF @Success = 1
			RAISERROR ('bcp of Trending_PrintingUtilization was not successful', 10, 1)
...
grant exec on [MPF_Trending_PrintingUtilization] to [imnet]
go