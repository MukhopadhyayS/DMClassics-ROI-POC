...
/*
Rem *************************************************************
Rem * Batch File Name:	ROI_MigrationFinalization.sql
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ROI_RequestMain]') AND type in (N'U'))
    ...
    END
ELSE
    IF NOT EXISTS (Select * from master.dbo.EPRS_Version where DB_NAME = 'ROI_Migration' and DB_VERSION = 'Final')
    BEGIN
        Insert master.dbo.EPRS_Version Values('ROI_Migration','Final',getdate())
    END
    ELSE
    BEGIN
      select 'echo Migration_Error: The final migration has been performed.'
    END
