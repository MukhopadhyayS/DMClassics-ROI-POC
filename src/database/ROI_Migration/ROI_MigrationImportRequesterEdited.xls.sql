...
Print 'Importing edited Requesters.  Processing file ROI_RequesterEdited' + @l_charNumberOfImportFiles + '.xls.'
...
+ @l_charNumberOfImportFiles +'.xls'',''SELECT * FROM [Sheet1$]'')'

--Select @l_vcharSQL
EXEC(@l_vcharSQL)

select count(*) AS 'The total number of edited Requesers is' from cabinet.dbo.ROI_RequesterEdited
