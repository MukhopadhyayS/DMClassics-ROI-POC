...
namespace McK.EIG.ROI.Client.Base.View.Common
{
    ...
    {
        private static readonly AmountTextBoxClass _editingControl = new AmountTextBoxClass();
            ...
            initialFormattedValue, DataGridViewCellStyle dataGridViewCellStyle)
        {
            ...
            AmountTextBoxClass ctl = DataGridView.EditingControl as AmountTextBoxClass;
            // Use the default row value when Value property is null.
            if (ctl != null)
            {
                amountTextBox = ctl.Controls[1] as TextBox;                
                amountTextBox.TextChanged += new EventHandler(amountTextBox_TextChanged);
                amountTextBox.Text = Convert.ToString(initialFormattedValue, System.Threading.Thread.CurrentThread.CurrentUICulture);
            }
        }
        ...
        {            
            ...
            {
                TextBoxEditingControl editControl = DataGridView.EditingControl as TextBoxEditingControl;
                if (editControl != null)
                {
        ...
        protected override void Paint(Graphics graphics, Rectangle clipBounds, Rectangle cellBounds, int rowIndex, DataGridViewElementStates cellState, object value, object formattedValue, string errorText, DataGridViewCellStyle cellStyle, DataGridViewAdvancedBorderStyle advancedBorderStyle, DataGridViewPaintParts paintParts)
        {
            ...
            Point center = new Point(cellBounds.X - 6, cellBounds.Y + 3);
            Point tbcenter = new Point(cellBounds.X + 22, cellBounds.Y);
            Rectangle rect = new Rectangle(center, size);
            Rectangle tbrect = new Rectangle(tbcenter, new Size(93, 21));
            TextRenderer.DrawText(graphics,
                                    "$",
                                    font,
                                    rect, Color.Black);
            double amount;
            if (string.IsNullOrEmpty(formattedValue.ToString()) || (!double.TryParse(formattedValue.ToString(), out amount)))
            {
                formattedValue = 0;
            }
            //CR#367022 - Check the visual styles are enabled and paint the values accordingly
            if (TextBoxRenderer.IsSupported)
            {
                TextBoxRenderer.DrawTextBox(graphics,
                                            tbrect,
                                            Double.Parse(formattedValue.ToString(), System.Threading.Thread.CurrentThread.CurrentUICulture).ToString("n2", System.Threading.Thread.CurrentThread.CurrentUICulture),
                                            font,
    ...
    public class TextBoxEditingControl : AmountTextBoxClass, IDataGridViewEditingControl
    {
        ...
        public object EditingControlFormattedValue
        {
            get
            {
                return this.Controls[1].Text.ToString();
            }
            set
            {
                if (value is String)
                {
                    this.Controls[1].Text = value.ToString();
                }
            }
        }
