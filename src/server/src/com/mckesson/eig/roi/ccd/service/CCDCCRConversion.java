...
public class CCDCCRConversion {
    ...
    /*public String convertToPDF(File srcFile, File targetFile) throws Exception {
    	...
    	if (isCCDDocument(srcFile, false)) {
    	    ...
    	    convertCCD(is, out, false);
    	    return CCD;
    	} else if (isCCRDocument(srcFile, false)) {
    	    InputStream is = new FileInputStream(srcFile);
    	    convertCCR(is, out, false);
    	    return CCR;
    	} else if (isCCDDocument(srcFile, true)) {
    	    InputStream is = new FileInputStream(srcFile);
    	    convertCCD(is, out, true);
    	    return CCD;
    	} else if (isCCRDocument(srcFile, true)) {
    	    InputStream is = new FileInputStream(srcFile);
    	    convertCCR(is, out, true);
    	    return CCR;
    	} else {
    	    throw new ROIException(
    		    ROIClientErrorCodes.CCD_CCR_INVALID_FILE_FORMAT);
    	}
    }*/
    ...
    public String convertToPDF(String styleFile, File srcFile, File targetFile)
	    throws Exception {
	OutputStream out = new FileOutputStream(targetFile);
    ...
    private synchronized StreamSource getCcdXslt() {
    	...
    	if (_ccdXsltSrc == null) {
    	    _ccdXsltSrc = (String) SpringUtil
    		    .getObjectFromSpring(CCD_XSLT_FILE);
    	    if (_ccdXsltSrc == null) {
    		throw new ROIException(
    			ROIClientErrorCodes.CCD_UNABLE_TO_GET_XSLT);
    	    }
    	}
    ...
    private synchronized StreamSource getCcrXslt() {
    	...
    	if (_ccrXsltSrc == null) {
    	    _ccrXsltSrc = (String) SpringUtil
    		    .getObjectFromSpring(CCR_XSLT_FILE);
    	    if (_ccrXsltSrc == null) {
    		throw new ROIException(
    			ROIClientErrorCodes.CCR_UNABLE_TO_GET_XSLT);
    	    }
    	}
    	return new StreamSource(getInputStream(_ccrXsltSrc));
    }

    public void convertCCD(InputStream src, OutputStream out,
	    boolean convertSO8859, StreamSource xsltSource) throws UnsupportedEncodingException {
    ...
    public void convertCCR(InputStream src, OutputStream out,
	    boolean convertSO8859, StreamSource xsltSource) throws UnsupportedEncodingException {
    ...
    private void convertToPDF(StreamSource srcStream, StreamSource xsltSource,
	    OutputStream out) {
        
    	try {
    	    // Construct fop with desired output format
    	    _foUserAgent.setFOEventHandlerOverride(new PdfEventHandler(_foUserAgent, out));
    	    Fop fop = _fopFactory.newFop(MimeConstants.MIME_PDF, _foUserAgent,
    		    out);
    	    // Setup XSLT
    	    ...
    	    throw new ROIException(
    		    ROIClientErrorCodes.CCD_CCR_UNABLE_TO_CONVERT_PDF);
    	} finally {
    	    try {
    		out.close();
    	    } catch (Exception e) {
