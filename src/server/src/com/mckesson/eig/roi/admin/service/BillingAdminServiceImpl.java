...
@WebService(serviceName="BillingAdminService", endpointInterface="com.mckesson.eig.roi.admin.service.BillingAdminService",
targetNamespace="urn:eig.mckesson.com", portName="adminPort", name="BillingAdminServiceImpl")
public class BillingAdminServiceImpl extends BaseROIService
        implements
            BillingAdminService {

    private static final OCLogger LOG = new OCLogger(
            BillingAdminServiceImpl.class);
     ...
     */
      ...
      final String logSM = "createMediaType(mediaType)"; if (DO_DEBUG) {
      LOG.debug(logSM + ">>Start:" + mediaType); }
      
      try {
      
      MediaTypeDAO dao = (MediaTypeDAO) getDAO(DAOName.MEDIA_TYPE_DAO);
      BillingAdminServiceValidator validator = new
      BillingAdminServiceValidator();
      
      if (!validator.validateMediaType(mediaType, true, dao)) { throw
      ...
      user.getInstanceId(), dao.getDate());
      
      if (DO_DEBUG) { LOG.debug(logSM + "<<End :" + mediaTypeId); } return
      mediaTypeId; } catch (ROIException e) {
      LOG.error("ROIException occured in createMediaType",e); throw e; } catch
      (Throwable e) { LOG.error("Exception occured in createMediaType",e);
      throw new ROIException(e,
      ROIClientErrorCodes.MEDIA_TYPE_OPERATION_FAILED); } }
     ...
     */
      ...
      final String logSM = "deleteMediaType(mediaTypeId)"; if (DO_DEBUG) {
      ...
      if (!validator.validateMediaTypeDeletion(id, dao)) { throw
      validator.getException(); }
      
      MediaType mt = dao.deleteMediaType(id);
      
      User user = getUser(); auditAdmin(mt.toDeleteAudit(user.getFullName()),
      user.getInstanceId(), dao.getDate());
      
      if (DO_DEBUG) { LOG.debug(logSM + "<<End:"); } } catch (ROIException e) {
      LOG.error("ROIException occured in deleteMediaType",e); throw e; } catch
      (Throwable e) { LOG.error("Exception occured in deleteMediaType",e);
      throw new ROIException(e,
      ROIClientErrorCodes.MEDIA_TYPE_OPERATION_FAILED); } }
     ...
     */
      ...
      final String logSM = "retrieveMediaType(mediaTypeId)"; if (DO_DEBUG) {
      LOG.debug(logSM + ">>Start:"); }
      
      MediaTypeDAO dao = (MediaTypeDAO) getDAO(DAOName.MEDIA_TYPE_DAO);
      
      MediaType mediaType = dao.retrieveMediaType(mediaTypeId); long count =
      dao.getAssociatedBillingTierCount(mediaTypeId);
      mediaType.setIsAssociated(count > 0);
      
      if (DO_DEBUG) { LOG.debug(logSM + "<<End:" + mediaType); } return
      mediaType; }
     ...
     */
      ...
      if (detailedFetch) { return dao.retrieveAllMediaTypes(); }
      
      return dao.retrieveAllMediaTypeNames(); } catch (Throwable e) {
      LOG.error("Exception occured in retrieveAllMediaTypes",e); throw new
      ROIException(e, ROIClientErrorCodes.MEDIA_TYPE_OPERATION_FAILED); } }
     ...
     */
      ...
      final String logSM = "updateMediaType(mediaType)"; if (DO_DEBUG) {
      ...
      if (!validator.validateMediaType(mt, false, dao)) { throw
      validator.getException(); }
      
      setDefaultMediaTypeDetails(mt, dao, false); MediaType oldMT =
      retrieveMediaType(mt.getId());
      
      User user = getUser(); String comments =
      oldMT.toUpdateAudit(user.getFullName(), mt);
      
      MediaType uMT = dao.updateMediaType(mt, oldMT);
      
      auditAdmin(comments, user.getInstanceId(), dao.getDate());
      
      if (DO_DEBUG) { LOG.debug(logSM + "<<End:" + uMT); } return uMT; } catch
      (ROIException e) {
      LOG.error("ROIException occured in updateMediaType",e); throw e; } catch
      (Throwable e) { LOG.error("Exception occured in updateMediaType",e);
      throw new ROIException(e,
      ROIClientErrorCodes.MEDIA_TYPE_OPERATION_FAILED); } }
    ...
    public long createFeeType(FeeType feeType) {
        ...
        try {
            ...
            User user = getUser();
            auditAdmin(feeType.toCreateAudit(), user.getInstanceId(),
                    dao.getDate());
            ...
            throw new ROIException(e,
                    ROIClientErrorCodes.FEE_TYPE_PROCESS_FAILED);
        }
    }
    ...
    public void deleteFeeType(long feeTypeId) {
        ...
        try {
            ...
            User user = getUser();
            auditAdmin(fType.toDeleteAudit(name), user.getInstanceId(),
                    dao.getDate());
    ...
    /**
     * This method sets the default values while creating a new FeeType - This
     * method should set actual values
     *
     * @param feeType
     * @param dao
     ...
     *            decides to set values for creation/modification
     */
    private void setDefaultFeeTypeDetails(FeeType feeType, FeeTypeDAO dao,
            boolean isCreation) {
    ...
    public long createPaymentMethod(PaymentMethod paymentMethod) {
        ...
        try {
            ...
            PaymentMethodDAO dao = (PaymentMethodDAO) getDAO(
                    DAOName.PAYMENT_METHOD_DAO);
            BillingAdminServiceValidator validator = new BillingAdminServiceValidator();
            ...
            User user = getUser();
            auditAdmin(paymentMethod.toCreateAudit(), user.getInstanceId(),
                    dao.getDate());

            if (DO_DEBUG) {
                LOG.debug(logSM + "<<End: Created PaymentMethodId: "
                        + paymentMethodId);
            }
            ...
            throw new ROIException(e,
                    ROIClientErrorCodes.PAYMENT_METHOD_OPERATION_FAILED);
        }
    }
    ...
    public PaymentMethod retrievePaymentMethod(long paymentMethodId) {
        ...
        PaymentMethodDAO dao = (PaymentMethodDAO) getDAO(
                DAOName.PAYMENT_METHOD_DAO);
        PaymentMethod paymentMethod = dao
    ...
    public PaymentMethod updatePaymentMethod(PaymentMethod paymentMethod) {
        ...
        try {
            ...
            PaymentMethod originalPM = retrievePaymentMethod(
                    paymentMethod.getId());
            String comments = paymentMethod.toUpdateAudit(originalPM);
            PaymentMethod updatedPM = dao.updatePaymentMethod(paymentMethod,
                    originalPM);
    ...
    /**
     * This method sets the default values while creating/updating a Payment
     * Method
     *
     * @param paymentMethod
     *            Payment Method details to be set
     * @param payment
     *            method dao.
     * @param isCreate
     *            True for create and false for update
     */
    private void setPaymentMethodDetails(PaymentMethod paymentMethod,
            PaymentMethodDAO dao, boolean isCreate) {
    ...
    public long createBillingTemplate(BillingTemplate billingTemplate) {
        ...
        try {
            ...
            BillingTemplateDAO dao = (BillingTemplateDAO) getDAO(
                    DAOName.BILLING_TEMPLATE_DAO);
            BillingAdminServiceValidator validator = new BillingAdminServiceValidator();

            if (!validator.validateBillingTemplate(billingTemplate, true,
                    dao)) {
                throw validator.getException();
            }
            ...
            User user = getUser();
            auditAdmin(billingTemplate.toCreateAudit(), user.getInstanceId(),
                    dao.getDate());
            ...
            throw new ROIException(e,
                    ROIClientErrorCodes.BILLING_TEMPLATE_PROCESS_FAILED);
        }
    }
    ...
    public BillingTemplate retrieveBillingTemplate(long billingTemplateId) {
        ...
        BillingTemplateDAO dao = (BillingTemplateDAO) getDAO(
                DAOName.BILLING_TEMPLATE_DAO);
        BillingTemplate billingTemplate = dao
                .retrieveBillingTemplate(billingTemplateId);
        ...
        try {
            ...
            if (DO_DEBUG) {
                LOG.debug(logSM + "<<End: No of Records : "
                        + bTList.getBillingTemplates().size());
            }
            return bTList;
        } catch (Throwable e) {
    ...
    public void deleteBillingTemplate(long billingTemplateId) {
        ...
        try {
            ...
            if (!validator.validateBillingTemplateDeletion(billingTemplateId,
                    dao)) {
                throw validator.getException();
            }
            BillingTemplate billingTemplate = dao
                    .deleteBillingTemplate(billingTemplateId);

            User user = getUser();
            auditAdmin(billingTemplate.toDeleteAudit(), user.getInstanceId(),
                    dao.getDate());

            if (DO_DEBUG) {
                LOG.debug(logSM + "<<End : Billing Template Id "
                        + billingTemplateId);
            }
        } catch (ROIException e) {
    ...
    public BillingTemplate updateBillingTemplate(
        ...
        try {
            ...
            if (!validator.validateBillingTemplate(billingTemplate, false,
                    dao)) {
                throw validator.getException();
            }
            ...
            BillingTemplate origBillingTemplate = retrieveBillingTemplate(
                    billingTemplate.getId());
            FeeTypeDAO feeTypeDao = (FeeTypeDAO) getDAO(DAOName.FEE_TYPE_DAO);

            FeeTypesList feeTypes = feeTypeDao.retrieveAllFeeTypes();
            String comments = billingTemplate.toUpdateAudit(origBillingTemplate,
                    feeTypes);

            BillingTemplate updatedBillingTemplate = dao.updateBillingTemplate(
                    billingTemplate, origBillingTemplate);

            User user = getUser();
            auditAdmin(comments, user.getInstanceId(), dao.getDate());

            if (DO_DEBUG) {
                LOG.debug(logSM + "<<End: Updated Billing Template "
                        + updatedBillingTemplate);
            }
     ...
     *            decides to set values for creation/modification
     */
    private void setDefaultBillingTemplateDetails(
            BillingTemplate billingTemplate, boolean isNew,
        ...
        if (isNew) {
            ...
            for (Iterator<RelatedFeeType> iter = billingTemplate
                    .getRelatedFeeTypes().iterator(); iter.hasNext();) {
                btf = iter.next();
    ...
    public BillingTier createBillingTier(BillingTier billingTier) {
        ...
        try {
            ...
            BillingTierDAO dao = (BillingTierDAO) getDAO(
            ...
            User user = getUser();
            auditAdmin(billingTier.toCreateAudit(), user.getInstanceId(),
                    dao.getDate());
            ...
            throw new ROIException(e,
                    ROIClientErrorCodes.BILLING_TIER_OPERATION_FAILED);
        }
    }
    ...
    public BillingTiersList retrieveAllBillingTiers(boolean loadAssociation) {
        ...
        try {
            ...
            if (DO_DEBUG) {
                LOG.debug(logSM + ">>Start: Load BillingTier Assocation = "
                        + loadAssociation);
            }
            BillingTierDAO dao = (BillingTierDAO) getDAO(
                    DAOName.BILLING_TIER_DAO);
    ...
    public BillingTier updateBillingTier(BillingTier bt) {
        ...
        try {
            ...
            for (Iterator<PageLevelTier> it = bt.getPageLevelTier()
                    .iterator(); it.hasNext();) {
                ...
                if (pglt.getId() == 0) {
                    ...
                    for (Iterator<PageLevelTier> i = old.getPageLevelTier()
                            .iterator(); i.hasNext();) {

                        PageLevelTier pg = i.next();
                        if ((pg.getPage() == pglt.getPage())
                                && (pg.getPageCharge() == pglt.getPageCharge())
                                && (old.getId() == bt.getId())) {
                            pglt.setId(pg.getId());
    ...
    public void deleteBillingTier(long billingTierId) {
        ...
        try {
            ...
            User user = getUser();
            auditAdmin(bt.toDeleteAudit(bt.getName()), user.getInstanceId(),
                    dao.getDate());
     ...
     *
     * @param billingTier
     *            input Billing Tier object
     * @param dao
     *            Billing Tier DAO object
     * @param isNew
     *            true for creation, false for updating
     */
    private void setDefaultBillingTierValues(BillingTier billingTier,
            BillingTierDAO dao, boolean isNew) {
    ...
    public BillingTiersList retrieveBillingTiersByMediaTypeName(
        ...
        try {
            ...
            BillingTiersList btList = dao
                    .retrieveBillingTiersByMediaTypeName(mediaTypeName);

            if (DO_DEBUG) {
                LOG.debug(logSM + "<<End: No of records"
                        + btList.getBillingTiers().size());
            }
            return btList;
        } catch (Throwable e) {
            LOG.error(
                    "Exception occured in retrieveBillingTiersByMediaTypeName",
                    e);
            throw new ROIException(e,
                    ROIClientErrorCodes.BILLING_TIER_OPERATION_FAILED);
        }
      ...
      @Override public TaxPerFacility retrieveTaxPerFacility(long
      taxPerFacilityId) {
      
      final String logSM = "retrieveTaxPerFacility(taxPerFacilityId)"; if
      (DO_DEBUG) { LOG.debug(logSM + ">>Start:"); }
      
      TaxPerFacilityDAO dao = (TaxPerFacilityDAO)
      getDAO(DAOName.TAXPERFACILITY_DAO);
      
      TaxPerFacility salesTaxFacility =
      dao.retrieveTaxPerFacility(taxPerFacilityId); if (DO_DEBUG) {
      LOG.debug(logSM + "<<End:" + salesTaxFacility); } return
      salesTaxFacility; }
     

    
      @Override public TaxPerFacilityList retrieveAllTaxPerFacilities() {
      
      final String logSM = "retrieveAllTaxPerFacilities()"; if (DO_DEBUG) {
      LOG.debug(logSM + ">>Start: retrieving SalesTaxFacilities"); }
      
      try {
      
      TaxPerFacilityDAO dao = (TaxPerFacilityDAO)
      getDAO(DAOName.TAXPERFACILITY_DAO); return
      dao.retrieveAllTaxPerFacilities();
      
      } catch (Throwable e) {
      LOG.error("Exception occured in retrieveAllTaxPerFacilities",e); throw
      new ROIException(e, ROIClientErrorCodes.SALESTAX_OPERATION_FAILED); } }
    ...
    public TaxPerFacilityList retrieveAllTaxPerFacilitiesByUser(String userId) {
        ...
        try {
            ...
            TaxPerFacilityDAO dao = (TaxPerFacilityDAO) getDAO(
                    DAOName.TAXPERFACILITY_DAO);
            ...
            LOG.error(
                    "ROIException occured in retrieveAllTaxPerFacilitiesByUser",
                    roie);
            throw roie;
        } catch (Throwable e) {
            LOG.error("Exception occured in retrieveAllTaxPerFacilitiesByUser",
                    e);
            throw new ROIException(e,
                    ROIClientErrorCodes.SALESTAX_OPERATION_FAILED);
        }
    }
      ...
      final String logSM = "createTaxPerFacility(taxPerFacility)"; if
      (DO_DEBUG) { LOG.debug(logSM + ">>Start:" + taxPerFacility); }
      
      try {
      
      TaxPerFacilityDAO dao = (TaxPerFacilityDAO) getDAO(
      DAOName.TAXPERFACILITY_DAO); BillingAdminServiceValidator validator = new
      BillingAdminServiceValidator();
      
      if (!validator.validateTaxPerFacility(taxPerFacility, true, dao,
      getUser().getLoginId())) { throw validator.getException(); }
      
      TaxPerFacility defaultTaxFac = null; if (ROIConstants.Y ==
      taxPerFacility.getDefault()) {
      
      defaultTaxFac = dao.getDefaultTaxPerFacility(); if ((defaultTaxFac !=
      null) && (taxPerFacility.getId() != defaultTaxFac.getId())) {
      ...
      auditAdmin(taxPerFacility.toCreateAudit(), getUser().getInstanceId(),
      dao.getDate());
      
      if (DO_DEBUG) { LOG.debug(logSM + "<<End :" + mediaTypeId); } 
      return mediaTypeId; 
      } catch (ROIException e) {
      ...
      LOG.error("Exception occured in createTaxPerFacility", e); throw new
      ROIException(e, ROIClientErrorCodes.SALESTAX_OPERATION_FAILED); 
      } 
      }
      
      public TaxPerFacility updateTaxPerFacility(TaxPerFacility
      taxPerFacility) {
      
      final String logSM = "updateTaxPerFacility(taxPerFacility)"; if
      (DO_DEBUG) { LOG.debug(logSM + ">>Start:" + taxPerFacility); }
      ...
      TaxPerFacility updatedSTF = dao.updateTaxPerFacility(taxPerFacility,
      oldSTF);
      
      auditAdmin(comments, getUser().getInstanceId(), dao.getDate());
      
      if (DO_DEBUG) { LOG.debug(logSM + "<<End:" + updatedSTF); } return
      updatedSTF; } catch (ROIException e) {
      LOG.error("ROIException occured in updateTaxPerFacility", e); throw e; }
      catch (Throwable e) {
      LOG.error("Exception occured in updateTaxPerFacility", e); throw new
      ROIException(e, ROIClientErrorCodes.SALESTAX_OPERATION_FAILED); } }
      
     public void deleteTaxPerFacility(long taxPerFacilityId) {
      
      final String logSM = "deleteTaxPerFacility(taxPerFacilityId)"; if
      (DO_DEBUG) { LOG.debug(logSM + ">>Start:" + taxPerFacilityId); }
      
      try {
      
      TaxPerFacilityDAO dao = (TaxPerFacilityDAO) getDAO(
      DAOName.TAXPERFACILITY_DAO); ROIAdminServiceValidator validator = new
      ROIAdminServiceValidator();
      ...
      auditAdmin(salesTax.toDeleteAudit(), getUser().getInstanceId(),
      dao.getDate());
      
      if (DO_DEBUG) { LOG.debug(logSM + "<<End:"); } } catch (ROIException e) {
      LOG.error("ROIException occured in deleteTaxPerFacility", e); throw e; }
      catch (Throwable e) {
      LOG.error("Exception occured in deleteTaxPerFacility", e); throw new
      ROIException(e, ROIClientErrorCodes.SALESTAX_OPERATION_FAILED); } }
     ...
     */
    private void setDefaultTaxPerFacilityDetails(
            TaxPerFacility salesTaxFacility, TaxPerFacilityDAO dao,
            boolean isCreation) {
    ...
    public BillingPaymentInfo retrieveBillingAndPaymentInfo(
        ...
        try {
            ...
            PaymentMethodDAO payMethodDao = (PaymentMethodDAO) getDAO(
                    DAOName.PAYMENT_METHOD_DAO);
            ReasonDAO reasonDAO = (ReasonDAO) getDAO(DAOName.REASON_DAO);
            RequestorTypeDAO requestorTypeDAO = (RequestorTypeDAO) getDAO(
                    DAOName.REQUESTOR_TYPE_DAO);
            DeliveryMethodDAO deliveryMethodDao = (DeliveryMethodDAO) getDAO(
                    DAOName.DELIVERY_METHOD_DAO);
            CountryCodeConfigurationDAO countryDao = (CountryCodeConfigurationDAO) getDAO(
            ...
            FeeTypesList feeTypes = feeTypeDao
                    .retrieveAllFeeTypesByRequestorId(requestorTypeId);
            List<BillingTemplate> templatesList = retrieveBillingTemplateWithRelatedFeeTypes(
                    requestorTypeId);
            ...
            info.setDeliveryMethods(
                    deliveryMethodDao.retrieveAllDeliveryMethodNames());
            info.setPaymentMethods(
                    payMethodDao.retrieveAllPaymentMethodNames());

            // requests the requestor type of the given requestorTypeId
            RequestorType requestorType = requestorTypeDAO
                    .retrieveRequestorType(requestorTypeId);
            long count = requestorTypeDAO
                    .getAssociatedRequestorCount(requestorTypeId);
            requestorType.setIsAssociated(count > 0);
            info.setRequestorType(requestorType);

            info.setReasons(reasonDAO
                    .retrieveAllReasonsByType(ROIConstants.ADJUSTMENT_TYPE));
    ...
    /**
     * this method retrieves the list of all Billing templates with their
     * related feetypes
     *
     * @param requestorTypeId
     * @return list of all billing templste with the related feeTypes
     */
    private List<BillingTemplate> retrieveBillingTemplateWithRelatedFeeTypes(
            long requestorTypeId) {

        FeeTypeDAO feeTypeDao = (FeeTypeDAO) getDAO(DAOName.FEE_TYPE_DAO);
        BillingTemplateDAO billingTemplateDao = (BillingTemplateDAO) getDAO(
                DAOName.BILLING_TEMPLATE_DAO);

        // retrieves the list of all related feetypes
        List<RelatedFeeType> relatedFeeTypes = feeTypeDao
                .retrieveAllRelatedFeeByRequestorType(requestorTypeId);

        BillingTemplatesList billingTemplates = billingTemplateDao
                .retrieveAllBillingTemplatesByRequestorType(requestorTypeId);
        ...
        for (BillingTemplate template : billingTemplates
                .getBillingTemplates()) {
        ...
        for (RelatedFeeType feeType : relatedFeeTypes) {
            ...
            if (templateMap.containsKey(billingTemplateId)) {
                ...
                BillingTemplate billingTemplate = templateMap
                        .get(billingTemplateId);
