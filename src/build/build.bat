...
set LOGFILE=nightlybuild%DATE:~10%%DATE:~4,2%%DATE:~7,2%.log
set MAILLOGFILE=mail%DATE:~10%%DATE:~4,2%%DATE:~7,2%.log

set _config.file=%1

if "%_config.file%"=="" (
	set _config.file=build.properties
)

rem calls ant scripts to update build scripts
call ant -file update.xml -Dbuild.config.file.name=%_config.file%
...
call ant -file start.xml -Dbuild.config.file.name=%_config.file% -logfile %LOGDIR%\%LOGFILE%

call ant  -file send.email.xml -Dbuild.config.file.name=%_config.file% -Dlog.file=%LOGFILE% -logfile %LOGDIR%\%MAILLOGFILE%
