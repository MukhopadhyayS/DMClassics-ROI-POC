...
namespace McK.EIG.ROI.Client.Base.View.Common.Tree
{
	public partial class TreeViewAdv
	{
		...
		{
			if (_dragTimer == null)
				_dragTimer = new System.Threading.Timer(new TimerCallback(DragTimerTick), null, 0, 1000);
		}
		...
		private void SetDropPosition(Point pt)
		{
			...
			if (node != null)
			{
				...
				if (pos < TopEdgeSensitivity)
					_dropPosition.Position = NodePosition.Before;
				else if (pos > (1 - BottomEdgeSensitivity))
			...
			if (nodes != null && nodes.Length > 0)
			{
				...
				using (Graphics gr = Graphics.FromImage(bitmap))
				{
					...
					if (maxWidth > 0 && y > 0)
					{
                        ...
                        BitmapHelper.SetAlphaChannelValue(_dragBitmap);
					}
					else
						_dragBitmap = null;
				}
