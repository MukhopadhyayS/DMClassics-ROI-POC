...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[MPF_Trending_ScanningProductivity]') AND type in (N'P'))
DROP PROCEDURE [dbo].[MPF_Trending_ScanningProductivity]
GO
...
* 	have been supplied.
*****************************************************************
* Procedure Name: [MPF_Trending_ScanningProductivity]
* Description:   
*
* NOTES: 
* Revision History:
*	Name					Date		Changes
*	---------				-------		-------------
* 	Bhanu Vanteru			11/29/2012	Created
...
*/
/*
	UC5934 - View Trending Scanning Volume Measure
	UC5935 - View Trending Scanning User Measure

	Utilization of MPF Scanning Productivity
	...
	DATE						CABINET		USER_PRODUCTIVITY		date and time when					DATE
																	occurred
	PGS_SCANNED					CABINET		USER_PRODUCTIVITY		Number of pages scanned				PGS_SCANNED
	SCAN_TIME					CABINET		USER_PRODUCTIVITY		Scan time (in minutes)				SCAN_TIME
	==============================================================================================================================
*/

CREATE PROCEDURE [dbo].[MPF_Trending_ScanningProductivity]
...
DELETE Trending_ScanningProductivity where [date]<@TrendRetentionDate
...
SET @TrendDate = (select CONVERT(smalldatetime,CONVERT(char(10), (select max([date]) from Trending_ScanningProductivity) + 1, 101)))
...
DELETE Trending_ScanningProductivity where [date] >= @TrendDate
...
SET  @sqlcmd = 'del ' + @HBI_TargetDir + '\MPF_Trending_ScanningProductivity.txt'
...
INSERT Trending_ScanningProductivity
(
      [FACILITY_NAME],
      [FullName],
      [date],
...
SET  @sqlcmd = ' bcp [his].[dbo].[Trending_ScanningProductivity] out ' + rtrim(@HBI_TargetDir) + '\MPF_Trending_ScanningProductivity.txt -c -T -S' + @@SERVERNAME

EXEC @Success =  master..xp_cmdshell @sqlcmd --, NO_OUTPUT

IF @Success = 1
      RAISERROR ('bcp of Trending_ScanningProductivity was not successful', 10, 1)
...
GRANT exec on [dbo].[MPF_Trending_ScanningProductivity] to [IMNET]
go