...
/*
 	Procedure Name: [MPF_Trending_EmailingUtilization]
 	Description:   
	...
	Srinivasan Ramaswamy		01/01/2013		Replaced Total Users with UserinstanceId 
	...
	Srinivasan Ramaswamy		05/09/2013		While counting users with Email Access from 'User_Security' table, query now ensures that that user is present in 'User_Facility' table table as well
...
/*
	UC5957 - View Trending Emailing Utilization User Measure
	...
	Occurred			AUDIT		AUDIT_TRAIL				Datetime when Emailing						date
	UserInstanceId		AUDIT		AUDIT_TRAIL				User who emailed				 			UserInstanceId
	USER_ID				CABINET		User_Security			Total number of users in a	facility		TotalUsersWithAccess
															who have access to emailing documents
...
*/
	...
	IF EXISTS ( 
		...
		FROM 
			sys.objects 
		WHERE 
			object_id = OBJECT_ID(N'[dbo].[MPF_Trending_EmailingUtilization]') AND 
			type in (N'P')
	)
	DROP PROCEDURE [dbo].[MPF_Trending_EmailingUtilization]
	GO


	CREATE PROCEDURE 
		[dbo].[MPF_Trending_EmailingUtilization]
	...
	BEGIN TRY
		...
		IF @TrendDate is null -- if null add rows to Trend Table using max(date) in Trend Table
			SET @TrendDate = (
				SELECT CONVERT(smalldatetime,
					CONVERT(char(10), 
					(SELECT MAX([DATE]) FROM Trending_EmailingUtilization with (nolock)) + 1, 101)))

		IF @TrendDate IS NULL or @TrendDate < @TrendRetentionDate -- no parameter and no max(date)
			SET @TrendDate = @TrendRetentionDate

		DELETE Trending_EmailingUtilization where [date] >= @TrendDate
		...
		SET  @sqlcmd = 'del ' + @HBI_TargetDir + '\MPF_Trending_EmailingUtilization.txt'
		...
		SET  @sqlcmd = ' bcp [his].[dbo].[Trending_EmailingUtilization] out ' + 
			rtrim(@HBI_TargetDir) + 
			'\MPF_Trending_EmailingUtilization.txt -c -T -S' + @@SERVERNAME
		...
		IF @Success = 1
			RAISERROR ('bcp of Trending_EmailingUtilization was not successful', 10, 1)
...
grant exec on MPF_Trending_EmailingUtilization to [IMNET]
go