...
--/****** Object:  StoredProcedure [dbo].[ROI_Generate_TurnAroundTimeReport]    Script Date: 11/11/2008  ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ROI_Generate_TurnAroundTimeReport]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[ROI_Generate_TurnAroundTimeReport]


USE [cabinet]
GO
/****** Object:  StoredProcedure [dbo].[ROI_Generate_TurnAroundTimeReport]    Script Date: 07/06/2009 12:53:42 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[ROI_Generate_TurnAroundTimeReport]
@requestorTypes xml,
@userIds xml,
...
Declare @TempResults table (Actor varchar(max), RequestorType varchar(max), RequestorName varchar(max), TotalCount int, Disposition int, TAT int)
...
select cast(y.actor as varchar(8000)) Actor,
       ...
       cast(y.requestor_name as varchar(8000)) RequestorName,
...
group by y.actor,y.requestor_type,y.requestor_name, reqType
order by y.actor,y.requestor_type,y.requestor_name, reqType desc
...
order by y.actor,y.requestor_type,y.requestor_name,reqType desc
GO
GRANT EXECUTE ON [dbo].[ROI_Generate_TurnAroundTimeReport] TO [IMNET]
GO
