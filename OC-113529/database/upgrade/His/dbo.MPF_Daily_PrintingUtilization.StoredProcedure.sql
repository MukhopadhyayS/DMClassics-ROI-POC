...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[MPF_Daily_PrintingUtilization]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[MPF_Daily_PrintingUtilization]
GO
...
* 	have been supplied.
*****************************************************************
* Procedure Name: [MPF_Daily_PrintingUtilization]
* Description:   
*
* NOTES: 
* Revision History:
*	Name					Date		Changes
*	---------				-------		-------------
* 	Richard Brightwell		11/07/2012	Created
*	MD Meersma				11/30/2012	Code Review
*   Latonia Howard          03/06/2013	set User Department as 'Undefined' if  null, added "Undefined" and "ALL Documents" conditions for DocumentType 
...
*/
/*
	UC5952 - View Daily Printing Utilization Measure

	Utilization of MPF Printing
	...
	Occurred					AUDIT		AUDIT_TRAIL				Datetime when Printing				Occurred
																	event occurred
...
CREATE PROCEDURE [dbo].[MPF_Daily_PrintingUtilization]
...
SET  @sqlcmd = 'del ' + @HBI_TargetDir + '\MPF_Daily_PrintingUtilization.txt'
EXEC @Success =  master..xp_cmdshell @sqlcmd, NO_OUTPUT

IF OBJECT_ID('tempdb..##HBI_SUBSET_DAILY_PRINTINGUTILIZATION') IS NOT NULL
DROP TABLE ##HBI_SUBSET_DAILY_PRINTINGUTILIZATION
	...
	[TotalUsersWithAccess] = 
		CASE
			WHEN ROW_NUMBER() over (partition by Facility order by occurred) = 1
				THEN (select count(distinct(user_id)) from [cabinet].[dbo].[User_Security] us with (nolock)
			...
			ELSE 0
		END
into ##HBI_SUBSET_DAILY_PRINTINGUTILIZATION
...
SET  @sqlcmd = ' bcp ##HBI_SUBSET_DAILY_PRINTINGUTILIZATION out ' + rtrim(@HBI_TargetDir) + '\MPF_Daily_PrintingUtilization.txt -c -T -S' + @@SERVERNAME

EXEC @Success =  master..xp_cmdshell @sqlcmd --, NO_OUTPUT

IF @Success = 1
	RAISERROR ('bcp of ##HBI_SUBSET_DAILY_PRINTINGUTILIZATION was not successful', 10, 1)
...
GRANT exec on [dbo].[MPF_Daily_PrintingUtilization] to [IMNET]
go