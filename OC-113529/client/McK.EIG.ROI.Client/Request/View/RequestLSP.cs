...
namespace McK.EIG.ROI.Client.Request.View
{
    ...
    /// RequestLSP
    /// </summary>
    public class RequestLSP : ROILeftSidePane 
    {
        #region Methods
        ...
        {
            return new Type[] { typeof(RequestNavigationPane) };
        }

        internal override bool CanProcess(object sender, ApplicationEventArgs ae)
        {
            if (sender is RequestNavigationPane) return true;

            string action = ae.Info as string;
            return (sender is MenuPane && action.StartsWith(ROIConstants.RequestsModuleName));
        }
